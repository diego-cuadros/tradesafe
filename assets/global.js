  ...
  validateQtyRules() {
    ...
    if (this.input.min) {
      ...
      buttonMinus.classList.toggle(
        "disabled",
        finalValue <= parseInt(this.input.min)
      );
    }
    if (this.input.max) {
...
class MenuDrawer extends HTMLElement {
  ...
  bindEvents() {
    this.querySelectorAll("summary:not(.no-control-menu)").forEach((summary) =>
      summary.addEventListener("click", this.onSummaryClick.bind(this))
    );
    this.querySelectorAll("button:not(.no-control-menu)").forEach((button) =>
    ...
    function addTrapFocus() {
      trapFocus(
        ...
        detailsElement.querySelector("button") ||
          detailsElement.querySelector(":scope > summary")
      );
...
class HeaderDrawer extends MenuDrawer {
  ...
  openMenuDrawer(summaryElement) {
    ...
    document.documentElement.style.setProperty(
      ...
      `${parseInt(this.headerContent.getBoundingClientRect().bottom)}px`
    );
...
class BackButtonMenu extends HTMLElement {
  ...
  onButtonClick() {
    ...
    if (openingElements.length == 1) {
      ...
      lastElement
        .querySelector(":scope > div > div > .menu-drawer__close-button")
        .dispatchEvent(new Event("click"));
    }
  }
}
...
const interval = setInterval(() => {
  ...
  if (
    addSucessMessage &&
    addSucessMessage.innerHTML === "Product added to Wishlist"
  ) {
    const existingLink = addSucessMessage.querySelector("a");
    if (!existingLink) {
      const lineBreak = document.createElement("br");
      addSucessMessage.appendChild(lineBreak);

      const viewWishListMsg = document.createElement("a");
      viewWishListMsg.href = "/apps/wishlist/";
      viewWishListMsg.classList.add("th-wl-view-cart-link");
      viewWishListMsg.textContent = "View Wishlist";
      addSucessMessage.appendChild(viewWishListMsg);
    }
  }
